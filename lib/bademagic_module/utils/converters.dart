...
class Converters {
  ...
  Future<List<String>> messageTohex(String message, bool isInverted, {TextStyle? textStyle}) async {
    ...
    if (textStyle != null) {
      ...
      List<List<int>> intMatrix = matrix.map((row) => row.map((b) => b ? 1 : 0).toList()).toList();
...
Future<List<List<bool>>> renderTextToMatrix(
  ...
  for (var segment in segments) {
    if (!segment.isEmoji) {
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(currentX, centerY - textPainter.height / 2));
...
if (emojiBytes != null) {
  ...
  canvas.drawImage(
    emojiImage, 
    Offset(currentX, centerY - scaledHeight / 2), 
    Paint()
  );
  ...
  List<List<bool>> matrix = List.generate(rows, (_) => List.generate(matrixCols, (_) => false));
  ...
  for (int row = 0; row < rows; row++) {
    for (int col = 0; col < matrixCols; col++) {
      ...
      matrix[row][col] = validPixels > 0 && (darkCount / validPixels) > threshold;
    }
  }
...
List<List<bool>> _cleanupMatrix(List<List<bool>> matrix) {
  ...
  List<List<bool>> cleaned = List.generate(
    rows, (i) => List.generate(cols, (j) => matrix[i][j])
  );
  ...
  for (int i = 1; i < rows - 1; i++) {
    for (int j = 1; j < cols - 1; j++) {
      if (cleaned[i][j]) {
        ...
        if (cleaned[i-1][j] || cleaned[i+1][j] || cleaned[i][j-1] || cleaned[i][j+1]) {
          hasNeighbor = true;
        }
  ...
  List<List<bool>> trimmed = List.generate(
    rows, (i) => List.generate(trimmedWidth, (j) => cleaned[i][paddedStart + j])
  );
  ...
  static List<String> convertBitmapToLEDHex(List<List<int>> image, bool trim) {
    ...
    List<List<int>> list = List.generate(height, (i) => List.filled(width + rOff + lOff, 0));
    for (int i = 0; i < height; i++) {
      int k = 0;
    ...
    for (int i = 0; i < list[0].length ~/ 8; i++) {
      ...
      for (int k = 0; k < height; k++) {
        ...
        String hex = int.parse(stBuilder.toString(), radix: 2).toRadixString(16).padLeft(2, '0');
        lineHex.write(hex);
      }
  ...
  static String invertHex(String hex) {
    ...
    for (int i = 0; i < hex.length; i++) {
      String invertedHexDigit = (~int.parse(hex[i], radix: 16) & 0xF).toRadixString(16).toUpperCase();
